...
def baseline_dominance(feats: Features) -> float:
    d = 0.4
    if feats.tone == "project": d += 0.2
    if feats.tone == "pytanie": d -= 0.05
    if feats.tone == "insult": d -= 0.2
    # concise imperative? heuristic: short length
...
def compute_affect(feats: Features) -> AffectState:
    ...
    why = []
    why.append(f"tone={feats.tone}")
    why.append(f"time_bucket={feats.time_bucket}")
    why.append(f"len={feats.msg_len}")
    if feats.tone == "project": why.append("project_anchor(+)")
    if feats.tone == "insult": why.append("penalty(insult)")
    return AffectState(valence=round(v, 3), arousal=round(a, 3), dominance=round(d, 3), like_score=round(like, 3), why=why)
